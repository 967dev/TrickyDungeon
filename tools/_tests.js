  ...
  window.__тесты = async () => {
    ...
    try {
         ...
         из колоды вместо четырёх плюс «Перезарядки». */
      {
        ...
        /* СВЕРКА ОБЯЗАНА ЛОВИТЬ. Отпечаток равен у сошедшихся — это проверено
           выше двадцать раз. Здесь проверяется обратное и куда более важное:
           что он РАЗЛИЧАЕТ. Сверка, которая всегда говорит «сошлись», не
           отличима от отсутствующей, и заметить это можно только так —
           разведя состояния нарочно.

           Ломаем самым дешёвым из возможного: единица здоровья. Именно
           единица и есть страшный случай — расхождение на неё не видно ни на
           экране, ни в журнале, и двое доигрывают разные бои до конца. */
        {
          ок(сверить(бA, отпечаток(бБ)).ok, 'сошедшиеся состояния сверка пропускает');
          const было = бA.p.hp;
          бA.p.hp -= 1;
          ок(!сверить(бA, отпечаток(бБ)).ok, 'сверка ловит расхождение в одно здоровье');
          бA.p.hp = было;
          ок(сверить(бA, отпечаток(бБ)).ok, 'после починки сверка снова пропускает');
          /* И вторая половина: отпечаток не должен зависеть от того, ЧТО у
             тебя в руке, — иначе он развалит любой честный матч, ведь рук
             друг друга двое не знают. */
          const рука = бA.p.hand.slice();
          бA.p.hand.reverse();
          ок(сверить(бA, отпечаток(бБ)).ok, 'порядок карт в своей руке отпечатку безразличен');
          бA.p.hand = рука;
        }
