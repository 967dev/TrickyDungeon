...
function колодаЦвета(el, t) {
  /* СНАЧАЛА СТАРШИЕ И ПЛОТНЫЕ. Раньше здесь брались первые десять карт цвета
     в порядке массива — при 37 картах это случайно совпадало со «всеми, что
     есть» (эфира ровно 10, все и влезали). Стоило пулу вырасти, и та же
     строка стала собирать эфирную колоду тира 3 из одних карт НУЛЕВОГО тира:
     эфир показал 0% на девятом этапе, и это был артефакт сборщика.
     Строка меряет потолок — «как собрал бы человек ради цепочек», — значит и
     брать надо лучшее доступное. */
  const свои = CARDS.filter(c => !c.noColl && c.t <= t && c.el === el && (c.ty === 'u' || c.ty === 's'))
    .slice().sort((a, b) => (b.t - a.t) || (((b.a||0)+(b.h||0))/Math.max(1,b.c) - ((a.a||0)+(a.h||0))/Math.max(1,a.c)));
  const d = [];
