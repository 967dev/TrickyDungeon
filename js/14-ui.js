...
/* ================= справка ================= */
/* Каталог показывает ТОЛЬКО открытые карты. Раньше в нём лежали все тридцать
   семь с полным описанием эффекта — то есть справка выдавала содержимое паков
   целиком, ещё до первой крутки. Это ровно та же беда, что была в табло
   миссий: имена боёв, до которых игрок не дошёл, обесценивали их открытие.
   Правило то же и по той же причине.
   Закрытые не прячем совсем: под каждым тиром стоит счётчик «ещё N не
   открыто». Он говорит, что тир не пуст и что коллекцию есть куда собирать, —
   но не говорит, ЧТО именно там лежит. */
function catalogHTML(){
  return TIER_NAMES.map((name,ti)=>{
    const всеТира=COLLECTIBLE.filter(c=>c.t===ti);
    const cards=всеТира.filter(c=>(S.inv&&S.inv[c.id])>0);
    const закрыто=всеТира.length-cards.length;
    ...
    return `<h3>${name} ${'★'.repeat(ti+1)}</h3>`+cards.map(c=>{
      ...
      if(c.eff){const e=c.eff;
        const m={dmg:`${e.v} урона цели`,healHero:`лечение героя ${e.v}`,healAll:`лечение всех ${e.v}`,
          ...
          reflect:'отражает последний урон от карты'};
      ...
      +(закрыто?`<div class="catRow catLock" style="--cat:${hex}">
        ещё <b>${закрыто}</b> ${plural(закрыто,'карта','карты','карт')} этого тира не ${plural(закрыто,'открыта','открыты','открыто')}
      </div>`:'')
...
function openInfo(tab){
  ...
  const C={
   battle:`<h3>КАК ПОБЕДИТЬ</h3>
    ...
    <h3>СТАРТОВАЯ РУКА И ЗАМЕНА</h3>
    <p><b>Колода тасуется заново перед каждым боем.</b> Порядок, в котором ты складывал её в «КОЛОДЕ», ни на что не влияет: два боя одной и той же колодой раздадут разное.</p>
    <ul>
    <li>На старте у тебя <b>4 карты из колоды</b> плюс <b>«Перезарядка»</b> — её дают сверх колоды, в самой колоде её нет, и на экране замены она не показывается: менять её не на что.</li>
    <li>Перед боем показывают эти четыре карты и дают <b>вернуть в колоду до двух</b>, взяв взамен другие. Тапни карту, чтобы пометить.</li>
    <li>Возвращённая карта уходит в колоду, колода тасуется — и <b>та же карта может прийти снова</b>. Замена не гарантирует, что станет лучше.</li>
    <li>Дальше — по одной карте в начале каждого твоего хода.</li>
    <li>У врага на старте <b>пять карт</b>: он ходит вторым, и лишняя карта — плата за это. Свою руку он не показывает.</li>
    </ul>
    <h3>ЦЕПОЧКИ СТИХИЙ</h3>
    <p>У каждой карты есть стихия — её значок виден на карте и в счётчике под твоей панелью. <b>Три карты одной стихии подряд</b> — и стихия срабатывает. Карта не в цвет обрывает набор, счётчик начинается заново. Считается выкладывание карты, а не удар. Набор не сбрасывается в конце хода.</p>
    <ul>
    <li><span class="kw">ОГОНЬ</span> · <b>ПРОЖАРКА</b> — 2 урона всем вражеским юнитам.</li>
    <li><span class="kw">ЛЁД</span> · <b>ЗАМОРОЗКА БОЛИ</b> — урон в лицо не проходит ход противника, вражеским юнитам −1 атаки.</li>
    <li><span class="kw">ЭФИР</span> · <b>ПУСТОТНЫЙ ВЗРЫВ</b> — врагу −2 маны на следующем ходу.</li>
    <li><span class="kw">ВОЛЬТА</span> · <b>ВЫСОКОЕ НАПРЯЖЕНИЕ</b> — +1/+1 своему юниту на выбор, навсегда.</li>
    <li><span class="kw">СТАЛЬ</span> · <b>ПУЛЕНЕПРОБИВАЕМЫЙ</b> — свой юнит на выбор не получает урона до конца хода противника.</li>
    </ul>
    <p>Вольта и сталь просят цель: ход не закончится, пока не ткнёшь в своего юнита. Цепочки собирает и противник — его счётчик виден под верхней панелью.</p>`,
   cards:`<h3>ТВОИ КАРТЫ И ИХ ЭФФЕКТЫ</h3>
    <p>★ — редкость. Формат: <b>имя</b> · стоимость маны · атака/здоровье · ключевые слова · эффект.</p>
    <p>Здесь только то, что <b>уже открыто</b>. Что лежит в закрытых — узнаешь из паков.</p>
    ${catalogHTML()}`,
   strat:`<h3>КАК УРОНИТЬ ЗДОРОВЬЕ СОПЕРНИКА</h3>
    ...
    ${/* Тут стояло «...или Графом Дыммом» — карты с таким именем в игре НЕТ.
         Осталась от старого набора и пережила его переименование. */''}
    <li><b>Кличи-урон добивают</b>: сначала размени обычными атаками, потом дожимай юнитом с боевым кличем-уроном по раненым.</li>
    ...
    <li><b>Собирай колоду по цвету</b>: чем больше в ней одной стихии, тем чаще замыкается цепочка. Одноцветная колода к поздним рейдам сильнее сборной «из лучшего» — именно на цепочках.</li>
    <li><b>Не рви набор ради мелочи</b>: на двух картах из трёх подумай, стоит ли выкладывать карту не в цвет. Иногда выгоднее подержать её ход.</li>
    </ul>
   ...
   raid:`<h3>КАРТА ГОРОДА</h3>
    <ul>
    <li>Район города — это акт. Внутри района <b>10 боёв</b>, каждый — партия против ИИ.</li>
    ...
    <li><b>Повторный заход в зачищенный рейд платит меньше</b> — четверть награды и не больше ${ПОВТОР_ПОТОЛОК} искр, а за поражение в нём не платят вовсе. Сколько даст именно этот заход — написано в строке этапа. Ферма после акта — <b>БАМбилово</b>: ${АРКАДА_НАГРАДА} искр за победу и соперник каждый раз новый.</li>
   ...
   odds:`<h3>ШАНС НА КАРТУ В ПАКЕ</h3>
    ...
    <h3>СЕКРЕТ</h3>
    ${/* Имя секретной карты показываем, только если она уже выпала. До этого
         справка называла её прямым текстом — то есть сама сдавала главный
