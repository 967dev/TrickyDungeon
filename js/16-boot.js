/* 16-boot.js — кристаллы, сторож боя, запуск. Подключается ПОСЛЕДНИМ: тут исполняемый код старта
...
setInterval(()=>{
  try{
    ...
    /* Замена стартовой руки: ход ещё не начат, и это не затык, а экран, который
       игрок читает. Считать его зависанием — значит начать бой у него за
       спиной. */
    if(B.mull){wdPhase=null;return}
    /* ИГРА ВДВОЁМ. Сторож держит правило «не фаза игрока не может длиться
       бесконечно» с пределом в двадцать секунд — это про соперника-автомата,
       который либо ходит, либо завис. Живой человек думает дольше двадцати
       секунд постоянно, и сторож отобрал бы у него ход посреди партии.
       За временем в дуэли следит свой таймер на тридцать секунд, а за тем,
       что соперник вообще на связи, — отсчёт минуты в 15-duel.js. Здесь
       сторожу делать нечего. */
    if(B.вдвоём){wdPhase=null;return}
...
PF.init();
renderMenu();
/* Точку «есть непрочитанное» зажигаем ОДИН РАЗ при запуске: кнопки справки
   лежат в разметке и никуда не пересоздаются, поэтому и перекрашивать их на
   каждом экране незачем. Гасит её сама справка, когда игрок прочтёт. */
обновитьЗначокНовостей();
/* Открыли по приглашению — сразу на экран матча, код там уже подставлен.
   Меню при этом успевает отрисоваться: если игра откажет (нет колоды), игрок
   должен видеть, куда идти дальше, а не пустой экран. */
if(typeof дКодИзАдреса==='function'&&дКодИзАдреса())позже(()=>go('duel'),0);
