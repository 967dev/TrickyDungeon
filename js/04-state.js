...
/* stage:0 — «Тренировка» стоит нулевым этапом, и новый игрок обязан пройти её
   прежде, чем откроется первый рейд.
   Полтора месяца тут стояла единица: обучение было недописано, и запирать за
   ним игру было нельзя. Теперь оно доигрывается целиком — все 23 шага, бой
   засчитывается, награда приходит (проверено прохождением, не на глаз), и
   держать дверь открытой больше незачем: без него человек идёт в бой, не зная
   ни про ману, ни про таунт, ни про цепочки. Сделать необязательным снова =
   поставить 1. */
...
const INV_CAP=20;
const DEF={v:SCHEMA,sparks:600,inv:{},deck:null,stage:0,done:{},hero:null,name:'',story:0,scenes:{},
  ...
  /* Какие подсказки в табло миссий игрок закрыл. По ВИДУ, а не одним
     выключателем: спрятать «загляни в ларёк» не должно прятать «собери
     колоду» — это разные сообщения, и второе важнее первого. */
  hints:{},
  /* БАМбилово: рекорд серии и ТЕКУЩАЯ серия. Текущую храним тоже, а не только
     рекорд: webview Телеграма выгружается при сворачивании, и серия из
     двенадцати побед, потерянная от чужого звонка, — это не «мелкая
     шероховатость», а повод больше не играть. */
  arc:{рек:0,стрик:0},
