...
function effDesc(c){
  ...
  if(!e){
    /* Ключевые слова СМОТРИМ ЗДЕСЬ, хотя их же расписывает kwLine рядом.
       Одна общая строка обещала «со следующего хода атакует» — а у карты с
       РАШем это ровно наоборот, и в разборе выходили две соседние строки,
       противоречащие друг другу: «РАШ атакует в тот же ход» и «атакует со
       следующего». Врали Дизель, Рэн и Ая Курона с самого начала; нашлось,
       когда в сталь добавили Аманду Армори.
       Заодно «эффект» заменён на «боевой клич»: ТАУНТ и РАШ — тоже эффекты с
       точки зрения игрока, и говорить про них «без эффекта» неправда. */
    if(c.kw&&c.kw.includes('rush'))
      return 'Без боевого клича — вся сила в статах и в <span class="kw">РАШе</span>: бьёт в тот же ход, когда вышел.';
    if(c.kw&&c.kw.includes('taunt'))
      return 'Без боевого клича — работа простая: встать стеной и держать удар. Хорошие статы за свои деньги.';
    return 'Без боевого клича — честный боец: ставь на поле, со следующего хода атакует. Хорошие статы за свои деньги.';
  }
  switch(e.k){
    ...
    /* Число НЕ показываем, хотя оно есть в состоянии боя. Причина та же, из-за
       которой существует откат(): рассказ отстаёт от расчёта, и «накоплено 6»
       засветилось бы раньше, чем игрок увидит сам удар на шесть. Игрок и так
       только что смотрел, чем в него попали. */
    case 'reflect':return pre+`<b>отражает последний урон, который тебе нанесли КАРТОЙ</b> — заклятием или боевым кличем. Удар существа не в счёт. Летит в любую цель на выбор: вражеский юнит или вражеский герой. Если картой ещё не били, отражать нечего.`;
