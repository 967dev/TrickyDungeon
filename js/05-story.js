...
/* Финал. Текст автора.
   Шепелявость выписана РОВНО В ДВУХ репликах: его вызове и ответе игрока,
   который передразнивает не слова, а манеру. Дальше не выписываем — читатель
   уже услышал, а «шшш» в каждой строке из шутки превращается в работу для
   глаз. Приём тот же, что в чате первого боя, где его называют «Шшшепелявый
   Шшшуллер»: шипение показывается утроенной «ш», а не подменой букв.
   Из этого же и вся острота: ник у него «Шепелявый Шулер», а произносят его —
   и он сам себя — с шипением. */
10:{who:'Шепелявый Шулер',foe:'shsh',
  pre:[
   ['t','{Готова|Готов} проиграть легендарному ШШ на глазах у тышшшяч моих подпишшшиков?!','calm'],
