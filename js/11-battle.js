  ...
  const рисовать=()=>{
    ...
    box.innerHTML=`<div class="mullBox">
      ...
      <div class="mullHand">${P.hand.map((id,i)=>{
        const c=byId(id), свой=id!=='zC0';
        return `<div class="mullC${выбор.has(i)?' меняем':''}${свой?'':' держим'}" data-i="${i}">
          ${cardHTML(c,{open:1,noAnim:1})}
          <span class="mullMark">${свой?(выбор.has(i)?'ЗАМЕНИТЬ':'оставить'):'не из колоды'}</span>
        </div>`}).join('')}</div>
      <div class="mullNote">Возвращённая карта попадает обратно в колоду и может прийти снова.
        «Перезарядка» не из колоды — её меняют не на что.</div>
      <div class="mullBtns">
    ...
    $$('.mullC[data-i]').forEach(el=>el.onclick=()=>{
      ...
      if(P.hand[i]==='zC0'){toast('«Перезарядку» дают сверх колоды — менять её не на что',1);return}
