...
/* Новое НАЖАТИЕ снимает заслонку, даже если съедать было нечего.
   suppressClick — «сожри следующий щелчок»: его ставит удавшееся
   перетаскивание и долгое нажатие, чтобы поверх разбора не отработал выбор
   цели. Расчёт на то, что щелчок обязательно придёт следом. Он приходит не
   всегда: onDragUp висит и на pointercancel (жест перехватила система —
   на телефоне это обычное дело), а отпускание вне окна щелчка тоже не даёт.
   Тогда флаг оставался поднятым НАВСЕГДА и съедал первое же настоящее
   нажатие игрока — где угодно, хоть в меню. Со стороны это ровно «кнопки
   срабатывают через раз, приходится тыкать дважды». */
...
function askGiveUp(){
  ...
  box.querySelector('#guYes').onclick=()=>{
    box.remove();
    /* СДАЧА — ЕДИНСТВЕННОЕ, О ЧЁМ СОПЕРНИК САМ НЕ ДОГАДАЕТСЯ. Обычный конец
       боя ему объявлять не нужно: он проигрывает те же сообщения и приходит к
       тому же итогу — если мой герой пал от его удара, он это уже знает. А
       сдача не следует ни из одного хода, и без объявления сдавшийся просто
       исчезал: второй ещё минуту ждал его хода и получал победу по правилу
       «соперник пропал». Исход верный, но приходит не тем путём и с не той
       надписью, а минута ожидания — на ровном месте.

       Бой закрываем ПРАВИЛАМИ (rOver), а не одним finish, и делаем это ДО
       объявления. Иначе отпечаток уедет: у принявшего «сдаюсь» бой уже
       кончен, а у меня ещё нет, и сверка развела бы состояния на последнем
       сообщении матча. finish ниже отработает как всегда — он и так зовётся
       по событию 'over' от правил, а второй раз его держит `итогПоказан`. */
    if(B&&B.вдвоём&&!B.over){
      B.дуэльИтог='сдался сам';
      rOver(B,false,true);
      дуэльОбъявить({т:'сдаюсь'});
    }
    finish(false,true);
  };
}
...
function dressBattle(st){
  ...
  { const c=(typeof CHATS!=='undefined'&&B)?CHATS[B.si]:null;
    /* У дуэли лицо приходит прямо в записи боя: это портрет героя, которого
       выбрал соперник. Через CHATS его не взять — там лица привязаны к
       сюжетным противникам по номеру этапа, а у дуэли номера нет. */
    const лицо=st.лицо||(c&&typeof ЛИЦА!=='undefined'?ЛИЦА[c.foe]:null);
    const ic=$('#eIc');
...
/* Заводка боя ВДВОЁМ. Отдельной функцией, а не условиями внутри startBattle:
   половина того, что там есть, к дуэли не относится вовсе — ни замена руки,
   ни соперник-автомат, ни снимок боя. Вплетать это условиями значило бы
   сделать хрупкими оба пути; ровно по той же причине отдельно живёт и
   БАМбилово.

   ЗАМЕНА РУКИ ЗДЕСЬ ЕСТЬ, и она стоила того самого согласования, которого я
   сперва избегал. Без неё дуэль начиналась с раздачи, которую нечем поправить,
   а в одиночном бою поправить можно — то есть игра вдвоём была ЖЁСТЧЕ игры с
   автоматом, хотя должна быть ровно наоборот.

   Обмениваться при этом нечем, и это главное. Замена не меняет НИЧЕГО из
   того, что видит соперник: рука остаётся той же длины, колода тоже, а карты
   в них у него и так рубашки. Значит отпечаток состояния не сдвигается, и всё
   согласование — это один сигнал «я готов» в обе стороны.

   Общее зерно замена тоже не рвёт, хотя тасует колоду: в дуэли из общего
   потока не берёт НИКТО. Карты не бросают кубик, соперник-автомат не
   запускается, а колода соперника у меня из рубашек — её порядок мне
   воспроизводить не нужно. Проверено поиском по случ() и pick().

   Снимок боя не пишем: восстанавливать матч, у которого второй конец давно
   ушёл, некуда. */
function startDuelBattle(соперник){
  dropBattleSnap();lockUI(0);clearFeed();
  B=newBattle(соперник,S.deck,{первый:Д.первый});
  go('battle');
  dressBattle(B.st);
  $('#bEnd').onclick=endTurn;
  $('#bGiveUp').onclick=askGiveUp;
  ...
  renderBattle();
  const бой0=B;
  /* Сначала замена руки — у обоих сразу, а не по очереди. Первый ход начнётся
     из дуэльГотов(), когда оба скажут «готов»: кто именно управится раньше,
     значения не имеет. */
  openMulligan(()=>{
    if(B!==бой0)return;
    дуэльГотов();
  },{срок:ДУЭЛЬ_МУЛЛИГАН_МС});
}

/* Первый ход дуэли. Отдельно от заводки, потому что зовётся не оттуда, а из
   момента «оба готовы» — а он приходит либо своей кнопкой, либо чужим
   сообщением, и заранее неизвестно, каким из двух. */
function дуэльНачатьХоды(){
  const бой0=B;
  позже(()=>{
    if(B!==бой0||!B||B.over)return;
    /* Первым ходит создавший комнату. У второго ход начинается у СОПЕРНИКА, и
       он просто ждёт: его собственный ход придёт сообщением «конец». */
    if(Д.первый){
      turnBanner('ТВОЙ ХОД!');
      startTurn('p');
      дуэльТаймерСтарт();
    }else{
      turnBanner('ХОД СОПЕРНИКА');
      startTurn('e');
      $('#bEnd').disabled=true;
    }
  },600);
...
function startBattle(si){
  ...
  /* Отложенный вызов привязан к СВОЕМУ бою. Иначе он переживает конец боя и
    ...
    позже(()=>{if(B!==бой0)return;startTurn('p');if(B.train)startTraining()},600);
  };
  /* В тренировке рука расписана сценарием — менять там нечего и незачем. */
  if(B.train)начать(); else openMulligan(начать);
}

/* ================= ЗАМЕНА СТАРТОВОЙ РУКИ =================
   Показывается перед каждым обычным боем. Механика стандартная для карточных
   игр (мулиган), но игроку, который её не встречал, она непонятна сама по
   себе — поэтому объяснение прямо на экране, а не в справочнике «где-нибудь».

   Что объясняем и почему именно это:
   — колода тасуется ПЕРЕД КАЖДЫМ боем. Без этой фразы замена читается как
     «игра дала мне плохие карты и извиняется»;
   — вернуть можно ДО ДВУХ карт. Иначе игрок ищет, где взять третью;
   — возвращённая карта уходит в колоду и МОЖЕТ ПРИЙТИ СНОВА. Это главное:
     иначе замена выглядит бесплатной, и человек справедливо удивится, увидев
     ту же карту опять. */
function openMulligan(after,опц){
  опц=опц||{};
  if(!B||B.over){after&&after();return}
  const бой=B, P=B.p;
  /* Флаг для сторожа боя: пока экран открыт, ход НЕ начат, и это законно.
     Без него сторож через двадцать секунд решает, что бой завис, и запускает
  ...
  document.body.appendChild(box);
  /* СРОК НА ВЫБОР. В одиночном бою его нет и быть не должно: думать над
     стартовой рукой можно сколько угодно, ждать некому. В дуэли ждёт живой
     человек, и без срока один игрок держит второго столько, сколько захочет.
     По истечении рука играется как есть — тем же правилом, что и у хода:
     время вышло, ход просто кончается. */
  let срокДо=опц.срок?Date.now()+опц.срок:0, часы=null;
  const снятьЧасы=()=>{if(часы)clearInterval(часы);часы=null};
  /* Одна дверь наружу на оба случая — кнопка и вышедший срок. Раньше выход
     был расписан прямо в обработчике кнопки; со вторым выходом это значило
     бы два одинаковых куска, которые однажды разойдутся. */
  const подтвердить=()=>{
    снятьЧасы();
    if(B===бой&&выбор.size){rMulligan(B,[...выбор]);B.ev.length=0;renderBattle()}
    if(B===бой)B.mull=0;
    box.remove();
    after&&after();
  };
  const рисовать=()=>{
    const осталось=МУЛИГАН_МАКС-выбор.size;
    box.innerHTML=`<div class="mullBox">
      <div class="mullT">СТАРТОВАЯ РУКА</div>
      <div class="mullS">Колода тасуется заново перед каждым боем. Не нравится рука —
        верни в колоду до двух карт и возьми взамен другие.${опц.срок
          ? ' Соперник сейчас делает то же самое.' : ''}</div>
      ${/* «Перезарядку» на этом экране НЕ ПОКАЗЫВАЕМ. Она лежит в руке всегда,
            в колоде её нет, менять её не на что — то есть слот, в котором она
            стояла, был единственным на экране, которым игрок не мог
            распорядиться. Друзья на плейтесте назвали это «бессмысленной
            картой в шаффле», и это точное слово: экран называется «выбери» и
            показывает то, чего не выбирают.
            Раздача кладёт её последней (см. newBattle), поэтому четыре первых
            места руки — это ровно четыре карты из колоды. */''}
      <div class="mullHand">${P.hand.map((id,i)=>{
        if(id==='zC0')return '';
        return `<div class="mullC${выбор.has(i)?' меняем':''}" data-i="${i}">
          ${cardHTML(byId(id),{open:1,noAnim:1})}
          <span class="mullMark">${выбор.has(i)?'ЗАМЕНИТЬ':'оставить'}</span>
        </div>`}).join('')}</div>
      <div class="mullNote">Возвращённая карта попадает обратно в колоду и может прийти снова.
        Сверху к этим четырём всегда ложится «Перезарядка» — она не из колоды и в замене не участвует.</div>
      <div class="mullBtns">
        <span class="mullCnt">${выбор.size?('меняем '+выбор.size+' · можно ещё '+осталось):('можно заменить '+осталось)}</span>
        ${опц.срок?'<span class="mullЧасы"></span>':''}
        <button class="btn pri" id="mullGo">${выбор.size?'ЗАМЕНИТЬ И В БОЙ ►':'ИГРАТЬ ЭТУ РУКУ ►'}</button>
      </div>
    </div>`;
    $$('.mullC[data-i]').forEach(el=>el.onclick=()=>{
      const i=+el.dataset.i;
      if(выбор.has(i))выбор.delete(i);
      else if(выбор.size>=МУЛИГАН_МАКС){toast('Больше двух карт за раз не меняют',1);return}
      else выбор.add(i);
      sfx.ui();рисовать();
    });
    $('#mullGo').onclick=()=>{sfx.ui();подтвердить()};
  };
  рисовать();
  /* Часы заводим ПОСЛЕ первой отрисовки и не перезаводим на каждой: рисовать()
     переписывает содержимое целиком, и таймер, привязанный к узлу, обнулялся
     бы от каждого касания карты — то есть срока не было бы вовсе. Поэтому
     таймер один, а ищет он свой узел заново. */
  if(срокДо)часы=setInterval(()=>{
    if(!document.body.contains(box)){снятьЧасы();return}
    const сек=Math.max(0,Math.ceil((срокДо-Date.now())/1000));
    const у=box.querySelector('.mullЧасы');
    if(у){у.textContent=сек+' с';у.classList.toggle('мало',сек<=10)}
    if(сек<=0)подтвердить();
  },250);
}
...
function startTurn(who){
  ...
  проиграть().then(()=>{
    ...
    /* В ДУЭЛИ АВТОМАТ НЕ ХОДИТ. На той стороне человек, и его ход придёт
       сообщением. Без этой проверки второй игрок получал бой, в котором за
       соперника играл ИИ: ход начинался у 'e', автомат его отыгрывал и
       возвращал ход — состояния расходились с первой же секунды.
       Поймалось живым прогоном на двух вкладках; в коде не видно, потому что
       startTurn выглядит как «просто начать ход». */
    if(B.вдвоём)return;
...
async function endTurn(){
  ...
  await пауза(200);
  if(B!==бой)return;
  /* В дуэли ход не передаётся автомату — он объявляется человеку, и дальше мы
     просто ждём. Ход соперника придёт сообщением и отыграется сам. */
  if(B.вдвоём){
    /* СНАЧАЛА свой ход соперника, ПОТОМ объявление. Порядок важен из-за
       сверки: у принимающего «конец» ЗАПУСКАЕТ его ход (rStartTurn внутри
       принятьХод), то есть его состояние после сообщения — это состояние с
       уже начатым ходом. Объяви я до startTurn, отпечаток уехал бы на целый
       добор и ману, и первая же сверка развела бы совершенно исправный матч. */
    startTurn('e');
    дуэльОбъявить({т:'конец'});
    дуэльТаймерСтоп();
    return;
  }
  startTurn('e');
...
function playCard(i,targetUnit){
  ...
  /* Номер карты снимаем ДО вызова правил: rPlayCard возвращает только {ok},
     а из руки карту к тому времени уже убрал. Сопернику надо сказать, ЧТО
     именно сыграно, — у него на этом месте лежит рубашка. */
  const idКарты=B.p.hand[i];
  ...
  B.sel=null;
  /* Свой ход уходит сопернику ЗДЕСЬ, а не на четырёх кнопках, которые его
     вызывают: одна точка — один способ забыть. Номер слота берём до вызова
     правил, они карту оттуда уже убрали. */
  дуэльОбъявить({т:'карта',с:i,id:idКарты,ц:targetUnit?targetUnit.uid:null});
  проиграть();
}
...
async function doAttack(side,u,tgt,opts){
  ...
  /* Только СВОЙ удар: этой же функцией проигрывается и чужой, пришедший по
     сети, и объявлять его обратно значило бы зациклить обмен. */
  if(side==='p')дуэльОбъявить({т:'удар',u:u.uid,ц:tgt&&tgt.hero?'hero':(tgt?tgt.uid:null)});
  await проиграть();
}
...
async function сыграть(e){
  ...
  case 'chain':{
    renderBattle();
    sfx.elem(e.el);
    const цвет=элЦвет(e.el);
    if(!e.fire){
      /* Тик — отдельный такт, а не побочный эффект вылета карты. Ждём только
         за врага: свой набор игрок собрал сам и знает, что положил. */
      const box=chainPulse(e.who);
      /* Двойка — единственный момент, когда игрок ещё может помешать: между
         второй картой врага и третьей у него есть целый ход. Молчать об этом
         значит делать залп неожиданностью КАЖДЫЙ раз, сколько бы игрок ни
         играл, — то есть отнимать у механики единственное, чем на неё можно
         ответить. */
      if(e.n===2)chainTag(box,e.who==='p'?'ЕЩЁ ОДНА — ЗАЛП':'У ВРАГА ЕЩЁ ОДНА — ЗАЛП',цвет);
      /* Первый тик (n=1) — самое неинтересное событие набора: цвет только
         назвался, помешать ещё нечему. Второй — тот, из-за которого весь этот
         код. Время делим по важности, а не ровно: иначе ход врага, играющего
         в цвет, распухает на секунду каждый ход. */
      if(e.who==='e')await пауза(e.n===2?560:200);
      return;
    }
    ...
    /* Плашка держится две секунды, а рассказ уходил дальше через полсекунды:
       числа ехали поверх непрочитанного имени. Ждём, пока прочитает. */
    await пауза(1000);
    return;
  }

  /* Пас врага. Пустой ход в журнале читался как поломка: между «ХОД 9 · ВРАГ»
     и «ХОД 10 · ТЫ» не было ни строки. Теперь сказано, и сказана причина. */
  case 'pass':
    blog(e.who,e.нечем?'— ходить нечем —':'— не стал ходить: невыгодно —','sys');
    return;

  case 'ward':{
    const враг=e.who==='p'?'e':'p';
    ...
    renderBattle();
    const цвет=элЦвет('ice');
    chainTag(e.who==='p'?$('#pChain'):$('#eChain'),'ЩИТ',цвет);
    /* Поимённо по списку из события, а не по всей доске: замёрзли те, у кого
       была атака, и метка обязана стоять ровно над ними. */
    (e.мёрзнут||[]).forEach(u=>{const el=unitEl(враг,u.uid);chainLit(el,цвет);chainTag(el,'−1 АТК',цвет)});
    await пауза(660);
    return;
  }
  ...
  case 'warded':
    ...
    sfx.elem('ice');
    renderBattle();
    chainTag(e.who==='p'?$('#pHp'):$('#eHp'),`ЩИТ · −${e.v}`,элЦвет('ice'));
    return;

  case 'roast':
    blog(e.who,`✦ прожарка: ${e.v} урона всем вражеским юнитам`,'chain');
    /* Урон покажут события dmgUnit, идущие следом, — но им нужен свой такт:
       без него первое попадание приходится в ту же долю секунды, что и
       объявление залпа, и читается как часть плашки. */
    await пауза(340);
    return;

  case 'void':{
    blog(e.who,`✦ пустотный взрыв: врагу −${e.v} маны на следующем ходу`,'chain');
    /* Единственный залп, чей след виден не сейчас, а ХОДОМ ПОЗЖЕ. Значит
       метка обязана стоять там, где недостача проявится, — на мане. */
    const враг=e.who==='p'?'e':'p';
    chainTag(враг==='p'?$('#pMana'):$('#eMana'),`−${e.v} МАНЫ · СЛЕД. ХОД`,элЦвет('ether'));
    await пауза(620);
    return;
  }
  ...
  case 'manaLost':
    ...
    renderBattle();
    chainTag(e.who==='p'?$('#pMana'):$('#eMana'),`−${e.v} МАНЫ`,элЦвет('ether'));
    await пауза(420);
    return;

  case 'immune':
    blog(e.side,`✦ ${e.u.card.n} не почувствовал ${e.v}`,'chain');
    chainTag(unitEl(e.side,e.u.uid),'БРОНЯ ДЕРЖИТ',элЦвет('steel'));
    return;

  case 'chainVoid':
    blog(e.who,'✦ залп пропал: на поле некому его принять','chain');
    if(e.who==='e')toast('Залп врага пропал: на его поле некому его принять');
    await пауза(420);
    return;
  ...
  case 'chainBuff':{
    ...
    renderBattle();
    const el=unitEl(e.who,e.u.uid),цвет=элЦвет('volta');
    chainLit(el,цвет);chainTag(el,`+${e.a}/+${e.h}`,цвет);
    sfx.heal();
    await пауза(620);
    return;
  }
  ...
  case 'chainImm':{
    ...
    renderBattle();
    const el=unitEl(e.who,e.u.uid),цвет=элЦвет('steel');
    chainLit(el,цвет);chainTag(el,'БРОНЯ',цвет);
    await пауза(620);
    return;
  }
...
function effText(e){
  ...
  switch(e.k){
    ...
    /* Ноль проговариваем вслух: молчание выглядело бы поломкой, а не пустым
       зеркалом. */
    case 'reflect':return e.v>0?`↳ ${c.n} → ${цель} · ${e.v} (отражено)`
                              :`↳ ${c.n} — отражать нечего`;
  }
  return e.k;
}
function effKind(e){return (e.k==='dmg'||e.k==='aoe'||e.k==='drain'||(e.k==='reflect'&&e.v>0))?'atk':''}
...
/* ================= ЗАЛП: КОГО КАСАЕТСЯ И НАСКОЛЬКО =================
   Друзья на плейтесте сказали дословно: «когда враг собирает цепочку —
   визуально это практически не читаемо, слишком быстрая анимация и не
   понятно какой эффект применился и на кого». Причин оказалось три, и они
   разные, поэтому лечатся по отдельности:
   ...
   — НАБОР не занимал времени вообще. Событие тика перерисовывало полоску и
     ...
     карты, а сама полоска это эмблема 13px и три точки по 7px. На своей
     стороне её хотя бы ищешь глазами, на вражеской не видно ничего;
   — ЗАЛП объявлялся плашкой на две секунды, а рассказ шёл дальше через 520мс.
     Значит числа на доске начинали ехать, пока игрок ещё читает имя залпа, и
     связать «ПРОЖАРКА» с «у трёх юнитов стало меньше здоровья» было нечем;
   — САМИ ЭФФЕКТЫ не показывались НИГДЕ, кроме строки журнала. Урон прожарки
     ещё шёл ударами, их видно. А щит, минус атаки, минус маны, +1/+1 и броня
     просто меняли число в тишине.

   Отсюда четыре вещи ниже: метка НА ЦЕЛИ (chainTag), обводка цели в цвет
   стихии (chainLit), заметный тик полоски (chainPulse) и остановки в самих
   событиях. */
function unitEl(side,uid){
  const row=side==='p'?$('#rowP'):$('#rowE');
  return row?row.querySelector(`.unit[data-uid="${uid}"]`):null;
}
const элЦвет=el=>(EL_COLS[el]||EL_COLS.steel)[0];
/* Метка над целью. Своя, а не popDmg: та всегда красная и всегда про урон, а
...
function chainTag(el,текст,цвет){
  ...
  d.className='chTag';d.textContent=текст;
  d.style.setProperty('--c',цвет||'#ffd52e');
  d.style.left=(r.left+r.width/2)+'px';
  d.style.top=(r.top+r.height*.2)+'px';
  document.body.appendChild(d);
  /* Прижимаем к экрану ПОСЛЕ вставки: до неё ширины нет, а она тут разная —
     «−1 АТК» это 50 пикселей, «У ВРАГА ЕЩЁ ОДНА — ЗАЛП» полтораста. Метка
     центрируется по цели, и полоска набора врага стоит у левого края панели:
     на 375px предупреждение уезжало бы за край, то есть самая важная из
     меток читалась бы хуже всех. */
  const ш=d.offsetWidth;
  if(ш){
    const центр=clamp(r.left+r.width/2, ш/2+6, Math.max(ш/2+6, innerWidth-ш/2-6));
    d.style.left=центр+'px';
  }
  setTimeout(()=>d.remove(),1250);
}
/* Обводка на самой клетке — ответ на «на кого». Обводкой (outline), а не
   анимацией: у .unit слот анимации уже занят — выходом, смертью и подсветкой
...
/* Тик набора. Полоску надо не «обновить», а показать: её размер выбран так,
...
function onEnemyTap(uidOrHero){
  ...
  /* Выбирать нечего — значит тап это ПРОСМОТР. Друзья на плейтесте попросили
     дословно: «добавить возможность прокликать карточку врага в бою, приблизить
     как свою». Разбор чужого юнита умело долгое нажатие (wireHold) с самого
     начала — но узнать о нём было негде, а на своей карте в руке работает
     обычный тап. Здесь тап был занят ТОЛЬКО выбором цели, то есть в остальное
     время не делал ничего: свободный жест, на который просился ровно этот
     смысл. Герой не карта — его тапом не разбирают. */
  if(!B.sel){
    if(uidOrHero!=='hero')openUnitCard(uidOrHero);
    return;
  }
  if(B.sel.type==='unit'){
...
function finish(win,forfeit){
  ...
  /* Дуэль — своей веткой, как и БАМбилово. Здесь нет ни награды, ни отметки о
     зачистке, ни продвижения по этапам: за игру с человеком игра не платит, и
     вплетать это в общий путь незачем. Комнату закрываем — матч кончился. */
  if(B.st.дуэль){
    if(win)S.stats.wins++;
    save();
    if(typeof дЗакрыть==='function')дЗакрыть();
    if(typeof дуэльТаймерСтоп==='function')дуэльТаймерСтоп();
    /* ПОЧЕМУ МАТЧ КОНЧИЛСЯ. Одного «ПОБЕДА!!» мало: половина концовок в дуэли
       наступает не от того, что герой пал, а от поступка соперника или от
       поломки связи — и без причины любая из них читается как сбой. Вариант
       «победа непонятно за что» ничем не лучше «поражения непонятно за что».

       Причину ставит тот, кто её знает: кнопка сдачи, приём чужой сдачи,
       остановка по расхождению, пропажа комнаты, молчание соперника. Обычный
       конец боя причины не имеет и не должен — там всё видно на поле.

       Снимаем ЗДЕСЬ, до отложенного показа: к его моменту B уже могут
       обнулить, о чём предупреждает строка с si выше. */
    const итог=B.дуэльИтог||'';
    const ПРИЧИНЫ={
      'сдался соперник':'соперник сдался',
      /* Своей сдаче отдельной строки не нужно: она уже в шапке, и повтор
         «ТЫ СДАЛСЯ / ты сдался» выглядит опечаткой. Строка причины нужна там,
         где шапка её НЕ объясняет, — «ПОБЕДА!!» сама по себе не говорит, за
         что. */
      'сдался сам':'',
      'разошлись':'состояния разошлись',
      'комната пропала':'комната пропала',
      'соперник пропал':'соперник не вышел на связь',
    };
    const причина=ПРИЧИНЫ[итог]||'';
    /* Своя сдача — это не «матч прерван». Прерван он у того, у кого поломка;
       у сдавшегося кончился он ровно так, как тот и решил. */
    const шапка=win?'ПОБЕДА!!':(итог==='сдался сам'?'ТЫ СДАЛСЯ':(forfeit?'МАТЧ ПРЕРВАН':'ПОРАЖЕНИЕ'));
    if(win){sfx.win();PF.notify('success');setMood('joy',3200);
      burst(innerWidth/2,innerHeight/2,['#ffd52e','#fff','#ff4fd8','#35f0ff'],70,1.8);
      bang(шапка,50,30);
    }else{sfx.lose();PF.notify('error');setMood('sad',3200);
      bang(шапка,50,30)}
    позже(()=>{
      const box=document.createElement('div');box.className='bResult';
      box.innerHTML=`<div class="bResBox">
        <div class="bResT ${win?'win':'lose'}">${шапка}</div>
        ${причина?`<div class="bResПричина">${причина}</div>`:''}
        <div class="bResS">игра вдвоём · искры за неё не начисляются</div>
        <div class="bResB">
          <button class="btn pri" id="dNew">ЕЩЁ МАТЧ ►</button>
          <button class="btn" id="dMenu">В МЕНЮ</button>
        </div></div>`;
      document.body.appendChild(box);
      const уйти=()=>box.remove();
      $('#dNew').onclick=()=>{уйти();go('duel')};
      $('#dMenu').onclick=()=>{уйти();go('menu')};
    });
    return;
  }
  if(B.st.аркада){
  ...
  /* Зачищен ли рейд ДО этого боя — снимаем сейчас: ниже отметка ставится, и
     после неё «первый раз» уже не отличить от «сотый». */
  const зачищен=!!S.done[B.si];
  let gained=наградаРейда(B.st,зачищен,win,forfeit);
  if(win){
    S.stats.wins++;
    if(!зачищен){S.done[B.si]=1;
  ...
  позже(()=>{
    ...
    box.innerHTML=`<div class="bResBox">
      ...
      ${/* «повтор» сказан вслух: иначе 300 искр в первый раз и 40 во второй
            читаются как поломка награды, а не как правило. */''}
      <div class="bResS">${B.st.n}${зачищен?' · повтор зачищенного':''} · <b>${gained?'+'+fmtN(gained)+' искр':'без награды'}</b> · побед: ${S.stats.wins}/${S.stats.battles}</div>
      <div class="bResB">
        ${win&&мВсёПройдено()
