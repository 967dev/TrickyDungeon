...
function cardHTML(c,opts={}){
  ...
  if(c.eff){const e=c.eff;
    /* НОВЫЙ ВИД ЭФФЕКТА ВПИСЫВАЕТСЯ В ЧЕТЫРЕ МЕСТА: правила (rEffect),
       разбор карты (effDesc), строка журнала (effText) и вот этот короткий
       текст на лице карты. Про четвёртое легко забыть — так и вышло с
       «Зеркальным щитом»: карта с эффектом честно написала на себе «без
       эффекта». Теперь это ловит проверка в группе 15. */
  ...
  /* Подписи под числами. Два цветных квадрата без подписи читались только
     тем, кто уже знает, что красный слева — сила удара: цвет и место
     конвенция, но конвенцию надо откуда-то узнать.

     АТАКА, а не УРОН, и это не вкусовщина. «Урон» стоял в двух местах разом и
     означал разное: подпись под числом 2 (сколько юнит бьёт каждый ход) и
     строка «урон 1» в рамке (разовый боевой клич при выходе). На Огнемётчике
     это читалось как ошибка — одно слово, два числа. Везде в игре, кроме этой
     подписи, оно и так называется атакой: и в обучении («3 атаки, 1
     здоровья»), и в справочнике («атака/здоровье»), и в описании заморозки
     («−1 атаки»). Так что выбивалась именно карта. */
  const atkHp=c.ty==='u'
    ?`<div class="cfAtkHp"><span><b class="a">${c.a}</b><i>АТАКА</i></span>`
  ...
  return `<div class="cWrap t${c.t}${c.ult?' sec':''}${S.foil?' foil':''}${opts.open?' open':''}" ${opts.noAnim?'style="animation:none"':''}>
   <div class="cFlip">
    ...
    <div class="cFace cFront${art?' art':''}" style="--tc:${hex}">
      ...
      ${/* Коробка с эффектом теперь ВСЕГДА, и у бойца без способностей в ней
            написано, что способностей нет. Прежде она просто не рисовалась, и
            четыре карты из тридцати семи выглядели так, будто у них текст
            потерялся. «Без эффекта» — это не пустое место, это сведение: карта
            проверена, прятать в ней нечего. */''}
      <div class="cfText">${kw?`<span class="cfKw">${kw}.</span> `:''}${eff||(kw?'':'без эффекта')}</div>
      ${/* Голос карты — ТОЛЬКО в увеличенном виде (opts.fl). В руке, в колоде
            и в списке коллекции он мешает: там карту выбирают по цене, силе и
            эффекту, а лишняя строка курсивом отодвигает как раз их. Показываем
            его там, где карту РАССМАТРИВАЮТ: в разборе во время боя и в
            карточке из коллекции.
            Ставить его запасным вариантом описания, как было когда-то, тоже
            нельзя: тогда голос доставался одним лишь бойцам без эффекта, а на
            остальных 26 картах его не видел никто. */''}
      ${opts.fl&&c.fl?`<div class="cfFl">${c.fl}</div>`:''}
      ${atkHp}
...
const CARD_ART=new Set([
  ...
  /* пятая партия, 9 сентября 2026 — первая партия стальных карт.
     Исходники пришли вертикальными, 1024×1536, и кадрировались под квадрат
     поштучно: у четырёх взято окно по вертикали, а «Зеркальный щит» вписан
     целиком с чёрными полями по бокам — у него остриё внизу, и срезать его
     значило бы испортить силуэт. */
  'c14','c15','s10','s11','L07',
]);
...
/* Цена пака 150, а не 100. Замерено моделью на настоящей гаче: за полный
   первый проход акта (2775 искр с бонусами первой зачистки, плюс распыл
   дублей) при сотне выходило 34 пака, 169 карт и 2,8 ЛЕГЕНДЫ — то есть
   колода из легенд собиралась за один заход, а она, по отдельному замеру,
   превращает середину акта в коридор: 97/95/93/89 побед против 88/86/80/69
   средней колодой.
   При 150 выходит 20 паков и 1,7 легенды. Полнота коллекции при этом почти
   не меняется (69% против 59%) — цена вообще слабо на неё влияет, зато на
   легенды влияет вдвое. Резать награды за бои дало бы то же число, но
   ощущается хуже: победа над боссом должна платить щедро.
   Гарант (PITY_HARD) не трогаем: он и так почти не срабатывает — счётчик
   сбрасывается, когда легенда приходит сама. Работает мягкий с 55-й карты. */
const PITY_START=55,PITY_HARD=90,ULTRA_IN=.10,PACK=150;
...
function renderGacha(){
  /* Цену в верхней строке ставит КОД, а не разметка: в разметке она была
     вписана числом «100 искр» и после подорожания пака врала бы игроку. */
  { const t=$('#tbPack'); if(t)t.textContent='пак: 5 карт · '+PACK+' искр' }
  crumb('экран-ларька');
  tearing=false;
  ...
  <div class="gZone" id="gZone">
    ...
    <button class="pack" id="pack" ${broke?'disabled':''}
      ...
      ${/* Цена НАКЛЕЙКОЙ поверх пачки. На самой картинке нарисовано «100» —
           она рисовалась, когда пак столько и стоил. Перерисовывать арт ради
           одной цифры дороже, чем накрыть её, и это ровно тот же приём, что с
           плакатом на щите и подписью на радаре: место под надпись есть,
           надпись ставит игра. Координаты сняты С КАРТИНКИ по жёлтым пикселям:
           плашка занимает 32.97..64.69% ширины и 73.05..81.44% высоты. */''}
      <span class="pPrice">⚡ ${PACK}</span>
